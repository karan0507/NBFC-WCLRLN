...
export class DocumentPendingComponent implements OnInit {
  ...
  onClickFetchXML(action, id) {
    ...
    this.https.fetchXMLData(data).subscribe(
      (res) => {
        console.log(res);
  ...
  getFormLoanData(tableFilter?) {
    ...
    if (this.selectedTabFilter !== "B2B" && this.selectedTabFilter !== "D2C") {
      data = {
        ...
        endpoint: "LoanApplication?stage_id=22",
        source: "Onboarding",
      };
    } else if (this.selectedTabFilter == "B2B") {
      data = {
        datapoint: "loan_application",
        endpoint: "LoanApplication?stage_id=22&app_prod_type=B2B",
        source: "Onboarding",
      };
    } else if (this.selectedTabFilter == "D2C") {
      data = {
        datapoint: "loan_application",
        endpoint: "LoanApplication?stage_id=22&app_prod_type=D2C",
        source: "Onboarding",
      };
    }
  ...
  handleOk(type?) {
    switch (type) {
      ...
      case "uploadDocument":
        ...
        uploadDoc.append(
          "application_id",
          this._currentModalData["application"]
        );
        ...
        if (this._currentModalData?.document_master?.id) {
          uploadDoc.append(
            "document_id",
            this._currentModalData?.document_master?.id
          );
        }
  ...
  exportData(file_formate?) {
    ...
    let data = {
      source: "Onboarding",
      datapoint: "export_application_by_stage",
      stage_id: 2,
    };
    if (this.partner) {
      // data['page'] = 1

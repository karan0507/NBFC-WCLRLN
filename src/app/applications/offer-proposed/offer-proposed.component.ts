...
export class OfferProposedComponent implements OnInit {
  ...
  constructor(
    ...
    public sanitize: DomSanitizer, private route: ActivatedRoute, private router: Router
  ) {
  ...
  handleOk(type?) {
    switch (type) {
      ...
      case "reject":
        ...
        let params = {
          source: "Onboarding",
          datapoint: "reject-section-offer",
          application: this._currentLoanDetails?.id,
          offer_id: this._currentLoanDetails?.offer_id,
          section: "offer proposed",
      ...
      case "verify":
        ...
        this.https.verifyLoanDocument(param).subscribe(
          (res: any) => {
            if (res?.success) {
              this.api_calling_loader["button"] = false;
              this.message.success(res?.message);
